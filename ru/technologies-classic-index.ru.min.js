//#region bundles/technologies-classic-index/technologies-classic-index.ru.js
(function() {
	/**
	* @module identify
	*/
	var counter = 0;
	var expando = "__" + +/* @__PURE__ */ new Date(), get = () => "uniq" + ++counter, identify = (obj) => {
		if (typeof obj === "object" && obj !== null || typeof obj === "function") {
			...
			return key in obj ? obj[key] : obj[key] = get();
		}
		return "";
	};
	function identify_vanilla_default(obj) {
		if (arguments.length) {
			if (arguments.length === 1) return identify(obj);
			const res = [];
			for (const arg of arguments) res.push(identify(arg));
	...
	*/
	var noop = () => {}, extend = (o1, o2) => {
		if (o2) for (const [key, val] of Object.entries(o2)) o1[key] = val;
		return o1;
	}, toStr = Object.prototype.toString, isFunction = (obj) => toStr.call(obj) === "[object Function]";
	function override(base, res, add) {
		for (const name of Object.keys(add)) {
			if (name === "__self") continue;
			const prop = add[name];
			if (isFunction(prop) && (!prop.prototype || !prop.prototype.__self) && prop.toString().indexOf(".__base") > -1) res[name] = ((name) => {
				const baseMethod = base[name] ? base[name] : name === "__constructor" ? res.__self.__parent : noop, result = function() {
	...
	*/
	function inherit() {
		const args = arguments, withMixins = Array.isArray(args[0]), hasBase = withMixins || isFunction(args[0]), base = hasBase ? withMixins ? applyMixins(args[0]) : args[0] : noop, props = args[hasBase ? 1 : 0] || {}, staticProps = args[hasBase ? 2 : 1], res = props.__constructor || hasBase && base.prototype && base.prototype.__constructor ? function() {
		...
		extend(res, base);
		res.__parent = base;
		const basePtp = base.prototype, resPtp = res.prototype = Object.create(basePtp);
	...
	inherit.self = function() {
		const args = arguments, base = Array.isArray(args[0]) ? applyMixins(args[0], args[0][0]) : args[0], props = args[1], staticProps = args[2], basePtp = base.prototype;
	...
	*/
	var objects_vanilla_default = {
		extend(target, source) {
			(typeof target !== "object" || target === null) && (target = {});
			for (let i = 1, len = arguments.length; i < len; i++) {
				const obj = arguments[i];
				if (obj) for (const [key, val] of Object.entries(obj)) target[key] = val;
			}
			return target;
		},
		isEmpty(obj) {
		...
		each(obj, fn, ctx) {
			for (const [key, val] of Object.entries(obj)) ctx ? fn.call(ctx, val, key) : fn(val, key);
		}
	};
	/**
	* @module functions
	...
	*/
	...
	var enqueueFn = (fn) => {
		...
		const fnsToCall = fns;
		fns = [];
		for (const fn of fnsToCall) fn();
	};
	var nextTick = typeof queueMicrotask === "function" ? (fn) => {
		enqueueFn(fn) && queueMicrotask(callFns);
	} : typeof process === "object" && process.nextTick ? (fn) => {
	...
	* @module events
	*/
	var storageExpando = "__" + +/* @__PURE__ */ new Date() + "storage";
	/**
	* @class Event
	...
	*/
	...
	var events_vanilla_default = {
		Emitter: inherit(
			/** @lends Emitter.prototype */
			{
				on: function(e, data, fn, ctx, _special) {
					if (typeof e === "string") {
						if (functions_vanilla_default.isFunction(data)) {
							ctx = fn;
							fn = data;
							data = void 0;
						}
						const id = identify_vanilla_default(fn, ctx);
						const storage = this[storageExpando] || (this[storageExpando] = {});
						const eventTypes = e.split(" ");
						for (const eventType of eventTypes) {
							const eventStorage = storage[eventType] || (storage[eventType] = {
					...
					} else for (const [key, val] of Object.entries(e)) this.on(key, val, data, _special);
					return this;
				},
							...
							for (const eventType of eventTypes) {
								...
								if (eventStorage) if (fn) {
									...
									if (ids.has(id)) {
										...
										else if (item === list.first) list.first = next;
										if (next) next.prev = prev;
										else if (item === list.last) list.last = prev;
										ids.delete(id);
									}
								} else delete this[storageExpando][eventType];
							}
						} else delete this[storageExpando];
					} else for (const [key, val] of Object.entries(e)) this.un(key, val, fn);
					return this;
				},
						...
						for (const eventType of eventTypes) {
							...
							if (eventStorage) {
								...
								while (item) {
									...
									e.data = item.data;
									if (item.fn.call(item.ctx || this, e, data) === false) {
										e.preventDefault();
										e.stopPropagation();
									}
	...
	* @type String
	*/
	var NAME_PATTERN$1 = "[a-zA-Z0-9-]+";
	function isSimple(obj) {
		const typeOf = typeof obj;
		return typeOf === "string" || typeOf === "number" || typeOf === "boolean";
	}
	function buildModPostfix$1(modName, modVal) {
		let res = "";
		if (modVal != null && modVal !== false) {
			res += MOD_DELIM$1 + modName;
			modVal !== true && (res += MOD_DELIM$1 + modVal);
		}
		return res;
	}
	function buildBlockClassName(name, modName, modVal) {
		return name + buildModPostfix$1(modName, modVal);
	}
	function buildElemClassName(block, name, modName, modVal) {
		return buildBlockClassName(block, void 0, void 0) + ELEM_DELIM$2 + name + buildModPostfix$1(modName, modVal);
	}
	var i_bem__internal_vanilla_default = {
		NAME_PATTERN: NAME_PATTERN$1,
		MOD_DELIM: MOD_DELIM$1,
		ELEM_DELIM: ELEM_DELIM$2,
		buildModPostfix: buildModPostfix$1,
		buildClassName(block, elem, modName, modVal) {
			...
			} else if (typeof modName !== "undefined") modName = void 0;
			else if (elem && typeof elem !== "string") elem = void 0;
			if (!(elem || modName)) return block;
			return elem ? buildElemClassName(block, elem, modName, modVal) : buildBlockClassName(block, modName, modVal);
		},
		buildClassNames(block, elem, mods) {
			...
			let res = elem ? buildElemClassName(block, elem, void 0, void 0) : buildBlockClassName(block, void 0, void 0);
			if (mods) {
				for (const [modName, modVal] of Object.entries(mods)) if (modVal) res += " " + (elem ? buildElemClassName(block, elem, modName, modVal) : buildBlockClassName(block, modName, modVal));
			}
			return res;
		}
	};
	...
	* @module i-bem
	*/
	var ELEM_DELIM$1 = i_bem__internal_vanilla_default.ELEM_DELIM;
	/**
	...
	*/
	function buildSpecialModFn(modVal, curModFn, prevModFn, condition) {
		return prevModFn || condition ? function(_modName, _modVal, _prevModVal) {
			let res1, res2;
			prevModFn && (res1 = prevModFn.apply(this, arguments) === false);
			(!condition || condition(modVal, _modVal, _prevModVal)) && (res2 = curModFn.apply(this, arguments) === false);
	...
	*/
	function modFnsToProps(prefix, modFns, props) {
		if (functions_vanilla_default.isFunction(modFns)) props[buildModFnName(prefix, "*", "*")] = modFns;
		else for (const [modName, modFn] of Object.entries(modFns)) if (functions_vanilla_default.isFunction(modFn)) props[buildModFnName(prefix, modName, "*")] = modFn;
		else {
			...
			for (const [modValKey, curModFn] of Object.entries(modFn)) {
				...
				if (modValPrefix === "!" || modValPrefix === "~" || modVal === "*") {
					...
					props[starModFnName] = buildSpecialModFn(modVal, curModFn, props[starModFnName], specialModConditions[modValPrefix]);
				} else props[buildModFnName(prefix, modName, modVal)] = curModFn;
			}
		}
	}
	function buildCheckMod(modName, modVal) {
		if (modVal === false) modVal = "";
		return modVal != null ? Array.isArray(modVal) ? function(block) {
			for (const val of modVal) if (checkMod(block, modName, val)) return true;
			return false;
		} : function(block) {
	...
	function checkMod(block, modName, modVal) {
		...
		return modVal === "*" ? block.hasMod(modName) || prevModVal != null : block.hasMod(modName, modVal) || prevModVal === modVal;
	}
	function convertModHandlersToMethods(props) {
		for (const [key, prefix] of [["beforeSetMod", "before"], ["onSetMod", "after"]]) if (props[key]) {
			modFnsToProps(prefix, props[key], props);
			delete props[key];
		}
	}
	function declEntity(baseCls, entityName, base, props, staticProps) {
		base || (base = entities$1[entityName] || baseCls);
		Array.isArray(base) || (base = [base]);
		if (!base[0].__bemEntity) {
			base = base.slice();
			base.unshift(entities$1[entityName] || baseCls);
		}
		props && convertModHandlersToMethods(props);
		let entityCls;
		entityName === base[0].getEntityName() ? (entityCls = inherit.self(base, props, staticProps))._processInit(true) : entityCls = entities$1[entityName] = inherit(base, props, staticProps);
		return entityCls;
	}
	/**
	* @class BemEntity
	...
	var BemEntity = inherit(
		/** @lends BemEntity.prototype */
		{
			__constructor: function(mods, params, initImmediately) {
				...
				* @readonly
				*/
				this.params = objects_vanilla_default.extend(this._getDefaultParams(), params);
				/**
				* @member {String} Unique entity ID
				* @private
				*/
				this._uniqId = this.params.uniqId || identify_vanilla_default(this);
				initImmediately !== false ? this._setInitedMod() : initFns.push(this._setInitedMod, this);
			},
			...
			setMod: function(modName, modVal) {
				const typeModVal = typeof modVal;
				if (typeModVal === "undefined") modVal = true;
				else if (typeModVal === "boolean") modVal === false && (modVal = "");
				else modVal = modVal.toString();
				...
				for (const prefix of ["before", "after"]) {
					for (const modVar of modVars) if (this._callModFn(prefix, modVar[0], modVar[1], modFnParams) === false) {
						needSetMod = false;
						break;
					}
					if (!needSetMod) break;
			...
			setMods: function(mods) {
				for (const modName of Object.keys(mods)) this.setMod(modName, mods[modName]);
				return this;
			},
			...
			toggleMod: function(modName, modVal1, modVal2, condition) {
				...
				else if (typeof modVal1 !== "boolean") modVal1 = modVal1.toString();
				if (typeof modVal2 === "undefined") modVal2 = "";
				else if (typeof modVal2 === "boolean") {
					condition = modVal2;
					modVal2 = "";
				} else modVal2 = modVal2.toString();
				const modVal = this.getMod(modName);
				(modVal === modVal1 || modVal === modVal2) && this.setMod(modName, typeof condition === "boolean" ? condition ? modVal1 : modVal2 : this.hasMod(modName, modVal1) ? modVal2 : modVal1);
				return this;
			},
			...
			_callModFn: function(prefix, modName, modVal, modFnParams) {
				...
				return this[modFnName] ? this[modFnName].apply(this, modFnParams) : void 0;
			},
		...
		/** @lends BemEntity */
		{
			...
			declMod: function(mod, props, staticProps) {
				...
				objects_vanilla_default.each(props, function(prop, name) {
					functions_vanilla_default.isFunction(prop) && (props[name] = function() {
						let method;
						if (checkMod(this)) method = prop;
						else {
							const baseMethod = basePtp[name];
							baseMethod && baseMethod !== prop && (method = this.__base);
						}
						return method ? method.apply(this, arguments) : void 0;
					});
				});
	...
	*/
	var Elem$1 = inherit(
		...
		/** @lends Elem */
		{
			...
			getEntityName: function() {
				return this._blockName + ELEM_DELIM$1 + this._name;
			}
		}
	);
	var i_bem_vanilla_default = {
		Block: Block$1,
		Elem: Elem$1,
		entities: entities$1,
		...
		declElem(blockName, elemName, base, props, staticProps) {
			...
			} else entityName = blockName + ELEM_DELIM$1 + elemName;
			if (typeof base === "object" && !Array.isArray(base)) {
				staticProps = props;
				props = base;
				base = void 0;
		...
		_runInitFns() {
			if (initFns.length) {
				const fns = initFns;
				initFns = [];
				for (let i = 0; i < fns.length; i += 2) fns[i].call(fns[i + 1]);
			}
		}
	};
	/**
	...
	*/
	var jquery__config_default$1 = { version: ">=4.0.0" };
	/**
	* @module jquery__config
	* @description Configuration for jQuery (desktop override).
	* Previously downgraded jQuery for IE < 9, no longer needed.
	*/
	function jquery__config_default(base) {
		return base;
	}
	var _module = jquery__config_default$1;
	_module = jquery__config_default(_module);
	/*!
	...
	* @type Function
	*/
	var jquery_default = jQueryFactory(window, true);
	/**
	* @module dom
	* @description some DOM utils
	*/
	var EDITABLE_INPUT_TYPES = new Set([
		"datetime-local",
		"date",
		"month",
		"number",
	...
	var dom_default = {
		...
		getFocused() {
			try {
				return jquery_default(document.activeElement);
			} catch (e) {}
		},
		...
		isFocusable(domElem) {
			...
			switch (domNode.tagName.toLowerCase()) {
				...
				case "button":
				case "textarea":
				case "select": return !domNode.disabled;
				case "a": return !!domNode.href;
			}
			return false;
		},
		isEditable(domElem) {
			...
			if (!domNode) return false;
			switch (domNode.tagName.toLowerCase()) {
				case "input": return EDITABLE_INPUT_TYPES.has(domNode.type) && !domNode.disabled && !domNode.readOnly;
				case "textarea": return !domNode.disabled && !domNode.readOnly;
				default: return domNode.contentEditable === "true";
			}
		}
	};
	/**
	...
	* @class BemCollection
	*/
	var BemCollection = inherit(
		/** @lends BemCollection.prototype */
		{
			...
			concat(...args) {
				const argsForConcat = args.map((arg) => arg instanceof BemCollection ? arg._entities : arg);
	...
	function buildComplexProxyFn(arrayMethodName, entityMethodName) {
		return function(...args) {
			return this._entities[arrayMethodName]((entity) => entity[entityMethodName].apply(entity, args));
		};
	}
	/**
	...
	*/
	...
	function buildMapProxyFn(entityMethodName) {
		return function(...args) {
			return this._entities.map((entity) => entity[entityMethodName].apply(entity, args));
		};
	}
	...
	*/
	var EventManager = inherit(
		/** @lends EventManager.prototype */
		{
			...
			on(e, data, fn, _fnCtx, _isOnce) {
				...
				(typeof event === "string" ? event.split(/\s+/) : [event]).forEach((singleEvent) => {
			...
			un(e, fn, _fnCtx) {
				...
				if (argsLen) {
					...
					const event = this._eventBuilder(e, params);
					(typeof event === "string" ? event.split(/\s+/) : [event]).forEach((singleEvent) => {
						if (argsLen === 1) this._unbindByEvent(this._storage.get(singleEvent), singleEvent);
						else {
							const fnId = identify_vanilla_default(fn, _fnCtx);
							...
							const bindSelector = params.bindSelector;
							let wrappedFn;
							if (wrappedFn = fnStorage && fnStorage.get(fnId)) fnStorage.delete(fnId);
				...
				} else this._storage.forEach((fnStorage, e) => this._unbindByEvent(fnStorage, e));
				return this;
			},
	...
	var buildForEachEventManagerProxyFn = (methodName) => {
		return function() {
	...
	*/
	var CollectionEventManager = inherit(
		...
		/** @lends EventManagerFactory.prototype */
		{
			...
			getEventManager(ctx, bindCtx, bindScope) {
				...
				if (!ctxStorage) {
					...
					if (isBindToInstance) ctx._events().on({
						modName: "js",
						modVal: ""
					}, () => {
				...
				return ctxStorage[storageKey] || (ctxStorage[storageKey] = this._createEventManager(ctx, params, isBindToInstance));
			},
			_buildEventManagerParams(bindCtx, bindScope, ctxSelector, ctxCls) {
				...
				if (bindCtx) {
					...
					if (bindCtx.jquery) {
						...
						res.key = identify_vanilla_default.apply(null, bindCtx.get());
						res.bindToArbitraryDomElem = true;
					} else if (bindCtx === winNode || bindCtx === docNode || typeOfCtx === "object" && bindCtx.nodeType === 1) {
						res.bindDomElem = jquery_default(bindCtx);
						res.key = identify_vanilla_default(bindCtx);
						res.bindToArbitraryDomElem = true;
					} else if (typeOfCtx === "object" && bindCtx.__self) {
						...
						if (typeOfCtx === "string") {
							...
							blockName = bindCtx.block ? bindCtx.block.getName() : ctxCls._blockName;
							elemName = typeof bindCtx.elem === "function" ? bindCtx.elem.getName() : bindCtx.elem;
							modName = bindCtx.modName;
							modVal = bindCtx.modVal === false ? "" : bindCtx.modVal;
						} else if (bindCtx.getName() === bindCtx.getEntityName()) blockName = bindCtx.getName();
						else {
							blockName = ctxCls._blockName;
							elemName = bindCtx.getName();
						}
						const entityName = i_bem__internal_vanilla_default.buildClassName(blockName, elemName);
						res.bindEntityCls = this._getEntityCls(entityName);
						res.bindSelector = "." + (res.key = entityName + i_bem__internal_vanilla_default.buildModPostfix(modName, modVal));
					}
				} else res.bindEntityCls = ctxCls;
				return res;
			},
	...
	*/
	var eventBuilder$1 = function(e) {
		return e;
	};
	var i_bem_dom__events_type_dom_default = { EventManagerFactory: inherit(
		i_bem_dom__events_default.EventManagerFactory,
		/** @lends EventManagerFactory.prototype */
			...
			function wrapperFn(fn) {
				return function(e) {
					...
					if (isInstance) instance = ctx;
						...
						const entityDomNode = jquery_default(e.target).closest(params.ctxSelector);
						entityDomNode.length && (instance = entityDomNode.bem(ctx));
					}
					if (instance) {
						params.bindEntityCls && (e.bemTarget = jquery_default(this).bem(params.bindEntityCls));
						fn.apply(instance, arguments);
					}
				};
			}
			return new this._eventManagerCls(params, wrapperFn, eventBuilder$1);
		} }
	) };
	/**
	* @module i-bem-dom__events_type_bem
	*/
	var EVENT_PREFIX = "__bem__", MOD_CHANGE_EVENT = "modchange", specialEvents = jquery_default.event.special, specialEventsStorage = {}, createSpecialEvent = function(event) {
		return {
			setup: function() {
				specialEventsStorage[event] || (specialEventsStorage[event] = true);
			},
			teardown: functions_vanilla_default.noop
		};
	}, eventBuilder = function(e, params) {
		const event = EVENT_PREFIX + params.bindEntityCls.getEntityName() + (typeof e === "object" ? e instanceof events_vanilla_default.Event ? e.type : i_bem__internal_vanilla_default.buildModPostfix(e.modName, e.modVal === false ? "" : e.modVal) : e);
		specialEvents[event] || (specialEvents[event] = createSpecialEvent(event));
		return event;
	};
	var i_bem_dom__events_type_bem_default = {
		emit: function(ctx, e, data) {
			let originalEvent;
			if (typeof e === "string") originalEvent = new events_vanilla_default.Event(e, ctx);
			else if (e.modName) originalEvent = new events_vanilla_default.Event(MOD_CHANGE_EVENT, ctx);
			else if (!e.target) {
				e.target = ctx;
				originalEvent = e;
			}
			const event = eventBuilder(e, { bindEntityCls: ctx.__self });
			specialEventsStorage[event] && ctx.domElem.trigger(event, [
				data,
				{
					fns: {},
			...
			{ _createEventManager: function(ctx, params, isInstance) {
				function wrapperFn(fn, fnCtx, fnId) {
					return function(e, data, flags, originalEvent) {
						...
						let instance, instanceDomElem;
						if (isInstance) {
							instance = ctx;
							instanceDomElem = instance.domElem;
						} else {
							instanceDomElem = jquery_default(e.target).closest(params.ctxSelector);
							instanceDomElem.length && (instance = instanceDomElem.bem(ctx));
						}
						if (!params.bindSelector && params.bindDomElem.index(e.target) < 0) return;
						if (instance && (!flags.propagationStoppedDomNode || !jquery_default.contains(instanceDomElem[0], flags.propagationStoppedDomNode))) {
	...
	* @type Map
	*/
	var uniqIdToDomElems = /* @__PURE__ */ new Map();
	/**
	* Storage for blocks by unique key
	* @type Map
	*/
	var uniqIdToEntity = /* @__PURE__ */ new Map();
	/**
	* Storage for DOM element's parent nodes
	* @type Map
	*/
	var domNodesToParents = /* @__PURE__ */ new Map();
	/**
	* Storage for block parameters
	* @type Map
	*/
	var domElemToParams = /* @__PURE__ */ new Map();
	/**
	* Storage for DOM nodes that are being destructed
	* @type Map
	*/
	var destructingDomNodes = /* @__PURE__ */ new Map();
	var entities = i_bem_vanilla_default.entities;
	var BEM_CLASS_NAME = "i-bem";
	var BEM_SELECTOR = "." + BEM_CLASS_NAME;
	var BEM_PARAMS_ATTR = "data-bem";
	var NAME_PATTERN = i_bem__internal_vanilla_default.NAME_PATTERN;
	var MOD_DELIM = i_bem__internal_vanilla_default.MOD_DELIM;
	var ELEM_DELIM = i_bem__internal_vanilla_default.ELEM_DELIM;
	var buildModPostfix = i_bem__internal_vanilla_default.buildModPostfix;
	var buildClassName = i_bem__internal_vanilla_default.buildClassName;
	var reverse = Array.prototype.reverse;
	var domEventManagerFactory = new i_bem_dom__events_type_dom_default.EventManagerFactory(getEntityCls);
	var bemEventManagerFactory = new i_bem_dom__events_type_bem_default.EventManagerFactory(getEntityCls);
	var bemDom;
	/**
	...
	*/
	function initEntities(domElem, uniqInitId, dropElemCacheQueue) {
		...
		for (const entityName of Object.keys(params)) {
			...
			elemName && ((dropElemCacheQueue[blockName] || (dropElemCacheQueue[blockName] = {}))[elemName] = true);
			initEntity(entityName, domElem, processParams(params[entityName], entityName, uniqInitId));
		}
	}
	/**
	...
	*/
	function initEntity(entityName, domElem, params, ignoreLazyInit, callback) {
		const domNode = domElem[0];
		if (destructingDomNodes.has(identify_vanilla_default(domNode))) return;
		...
		if (entity) {
			if (entity.domElem.index(domNode) < 0) {
				...
				objects_vanilla_default.extend(entity.params, params);
			}
			return entity;
		}
		uniqIdToDomElems.set(uniqId, uniqIdToDomElems.has(uniqId) ? uniqIdToDomElems.get(uniqId).add(domElem) : domElem);
		const parentDomNode = domNode.parentNode;
		if (!parentDomNode || parentDomNode.nodeType === 11) jquery_default.uniqueSort(uniqIdToDomElems.get(uniqId));
		const entityCls = getEntityCls(entityName);
		entityCls._processInit();
		if (ignoreLazyInit || params.lazyInit === false || !entityCls.lazyInit && !params.lazyInit) {
			ignoreLazyInit && domElem.addClass(BEM_CLASS_NAME);
	...
	function getEntityCls(entityName) {
		...
		const splitted = entityName.split(ELEM_DELIM);
		return splitted[1] ? bemDom.declElem(splitted[0], splitted[1], {}, { lazyInit: true }) : bemDom.declBlock(entityName, {}, { lazyInit: true });
	}
	/**
	...
	*/
	function processParams(params, entityName, uniqInitId) {
		params.uniqId || (params.uniqId = (params.id ? entityName + "-id-" + params.id : identify_vanilla_default()) + (uniqInitId || identify_vanilla_default()));
		return params;
	}
	/**
	...
	*/
	function getParams(domNode) {
		const uniqId = identify_vanilla_default(domNode);
	...
	*/
	function removeDomNodeFromEntity(entity, domNode) {
		...
		} else entity.domElem = entity.domElem.not(domNode);
	}
	/**
	...
	*/
	function storeDomNodeParents(domElem) {
		domElem.each(function() {
			domNodesToParents.set(identify_vanilla_default(this), this.parentNode);
		});
	}
	/**
	...
	*/
	function dropElemCacheForCtx(ctx, dropElemCacheQueue) {
		ctx.add(ctx.parents()).each((_, domNode) => {
			const params = domElemToParams.get(identify_vanilla_default(domNode));
			params && objects_vanilla_default.each(params, (entityParams) => {
	...
	*/
	function buildElemKey(elem) {
		...
		else if (functions_vanilla_default.isFunction(elem)) elem = { elem: elem.getName() };
		else if (functions_vanilla_default.isFunction(elem.elem)) elem.elem = elem.elem.getName();
		return {
			elem: elem.elem,
	...
	*/
	function getJqueryCollection(html) {
		if (typeof html === "string") return jquery_default(jquery_default.parseHTML(html, null, true));
		if (html && html.domElem) return html.domElem;
		return jquery_default(html);
	}
	/**
	...
	*/
	function validateBlockParam(Block) {
		if (typeof Block === "string" || typeof Block === "object" && typeof Block.block === "string") throw new Error("Block must be a class or description (block, modName, modVal) of the block to find");
	}
	/**
	...
	*/
	var BemDomEntity = inherit(
		/** @lends BemDomEntity.prototype */
		{
			__constructor: function(domElem, params, initImmediately) {
				...
				*/
				this._findBackRefs = [];
				uniqIdToEntity.set(params.uniqId || identify_vanilla_default(this), this);
			...
			_elems: function(Elem) {
				...
				if (elemsCache && key.mod in elemsCache) return elemsCache[key.mod];
				const res = (elemsCache || (this._elemsCache[key.elem] = {}))[key.mod] = this.findMixedElems(Elem).concat(this.findChildElems(Elem));
			...
			_elem: function(Elem) {
				...
				if (elemCache && key.mod in elemCache) return elemCache[key.mod];
				const res = (elemCache || (this._elemCache[key.elem] = {}))[key.mod] = this.findMixedElem(Elem) || this.findChildElem(Elem);
			...
			findChildElem: function(Elem, strictMode) {
				return strictMode ? this._filterFindElemResults(this._findEntities("find", Elem)).get(0) : this._findEntities("find", Elem, true);
			},
			...
			findParentElem: function(Elem, strictMode) {
				return strictMode ? this._filterFindElemResults(this._findEntities("parents", Elem))[0] : this._findEntities("parents", Elem, true);
			},
			...
			_findEntities: function(select, entity, onlyFirst) {
				const entityName = functions_vanilla_default.isFunction(entity) ? entity.getEntityName() : typeof entity === "object" ? entity.block ? entity.block.getEntityName() : typeof entity.elem === "string" ? this.__self._blockName + ELEM_DELIM + entity.elem : entity.elem.getEntityName() : this.__self._blockName + ELEM_DELIM + entity;
				const selector = "." + (typeof entity === "object" ? buildClassName(entityName, entity.modName, typeof entity.modVal === "undefined" ? true : entity.modVal) : entityName) + (onlyFirst ? ":first" : "");
				const domElems = this.domElem[select](selector);
				if (onlyFirst) return domElems[0] ? initEntity(entityName, domElems.eq(0), void 0, true)._setInitedMod() : null;
				const res = [];
				const uniqIds = {};
				domElems.each((i, domElem) => {
					const block = initEntity(entityName, jquery_default(domElem), void 0, true)._setInitedMod();
			...
			_domEvents: function(ctx) {
				const bindScope = ctx === document || ctx === window ? jquery_default(ctx) : this.domElem;
			...
			_emit: function(e, data) {
				if (typeof e === "object" && e.modName === "js" || this.hasMod("js", "inited")) i_bem_dom__events_type_bem_default.emit(this, e, data);
				return this;
			},
			_extractModVal: function(modName) {
				const domNode = this.domElem[0];
				let matches;
				domNode && (matches = String(domNode.className).match(this.__self._buildModValRE(modName)));
			...
			_onSetMod: function(modName, modVal, oldModVal) {
				...
				this._findBackRefs.forEach(function(ref) {
					...
					ref._dropElemCache(modVal === "" ? name : {
						elem: name,
						modName,
						modVal
					});
				...
				if (modName !== "js" || modVal !== "") {
					...
					this.domElem.each(function() {
						...
						(oldModVal === true ? classNameRE.test(className) : (" " + className).indexOf(" " + classNamePrefix + MOD_DELIM) > -1) ? this.className = className.replace(classNameRE, needDel ? "" : "$1" + modClassName) : needDel || jquery_default(this).addClass(modClassName);
					});
				}
			},
			_afterSetMod: function(modName, modVal, oldModVal) {
			...
			containsEntity: function(entity) {
				return dom_default.contains(this.domElem, entity.domElem);
			},
	...
	*/
	var Block = inherit(
		[i_bem_vanilla_default.Block, BemDomEntity],
	...
	*/
	var Elem = inherit(
		[i_bem_vanilla_default.Elem, BemDomEntity],
		/** @lends Elem.prototype */
		{ _block: function() {
	...
	*/
	jquery_default.fn.bem = function(BemDomEntity, params) {
	...
	*/
	function getEntityFromDom(domNode, BemDomEntity) {
		...
		const entityParams = getParams(domNode)[entityName];
		return entityParams && entityParams.uniqId ? uniqIdToEntity.get(entityParams.uniqId) || null : null;
	}
	bemDom = {
		scope: null,
		doc: null,
		...
		declBlock: function(blockName, base, props, staticProps) {
			if (!base || typeof base === "object" && !Array.isArray(base)) {
				staticProps = props;
				props = base;
				base = typeof blockName === "string" ? entities[blockName] || Block : blockName;
			}
			base = getEntityBase(Block, blockName, base);
			return i_bem_vanilla_default.declBlock(blockName, base, props, staticProps);
		},
		declElem: function(blockName, elemName, base, props, staticProps) {
			if (typeof blockName === "function") if (typeof elemName !== "string") {
				staticProps = props;
				props = base;
				base = elemName;
				elemName = blockName._name;
				blockName = blockName._blockName;
			} else blockName = blockName.getName();
			...
			return i_bem_vanilla_default.declElem(blockName, elemName, base, props, staticProps);
		},
		declMixin: i_bem_vanilla_default.declMixin,
		init: function(ctx) {
			ctx = typeof ctx === "string" ? jquery_default(ctx) : ctx || bemDom.scope;
			const dropElemCacheQueue = {};
			const uniqInitId = identify_vanilla_default();
			findDomElem(ctx, BEM_SELECTOR).each(function() {
				initEntities(jquery_default(this), uniqInitId, dropElemCacheQueue);
			});
			i_bem_vanilla_default._runInitFns();
		...
		_destruct: function(ctx, excludeSelf, destructDom) {
			...
			reverse.call(findDomElem(_ctx, BEM_SELECTOR)).each((_, domNode) => {
				const params = getParams(domNode);
				const domNodeId = identify_vanilla_default(domNode);
				destructingDomNodes.set(domNodeId, true);
				currentDestructingDomNodes.push(domNodeId);
				objects_vanilla_default.each(params, (entityParams) => {
					if (entityParams.uniqId) {
						const entity = uniqIdToEntity.get(entityParams.uniqId);
						entity ? removeDomNodeFromEntity(entity, domNode) : uniqIdToDomElems.delete(entityParams.uniqId);
					}
				});
				domElemToParams.delete(identify_vanilla_default(domNode));
			});
			destructDom && (excludeSelf ? ctx.empty() : ctx.remove());
		...
		initOnDom: function(domNode, Entity, params) {
			const domElem = domNode.jquery ? domNode : jquery_default(domNode);
			const entity = initEntity(typeof Entity === "string" ? Entity : Entity.getEntityName(), domElem, params, true);
			return entity ? entity._setInitedMod() : null;
		}
	};
	jquery_default(function() {
		bemDom.scope = jquery_default("body");
		bemDom.doc = jquery_default(document);
		bemDom.win = jquery_default(window);
	});
	var i_bem_dom_default = bemDom;
	/**
	* @module i-bem-dom__init
	*/
	function i_bem_dom__init_default(ctx) {
		return i_bem_dom_default.init(ctx);
	}
	/**
	* Auto initialization on DOM ready
	*/
	jquery_default(function() {
		nextTick(i_bem_dom__init_default);
	});
	/**
	* @module keyboard__codes
	*/
	var keyboard__codes_default = {
		BACKSPACE: 8,
		TAB: 9,
		ENTER: 13,
		CAPS_LOCK: 20,
	...
	var input_default = i_bem_dom_default.declBlock("input", {
	...
	var search_default = i_bem_dom_default.declBlock("search", {});
	i_bem_dom_default.declBlock("form", { onSubmit: function(e) {
		var event = new events_vanilla_default.Event("submit");
		this._emit(event);
	...
	i_bem_dom_default.declBlock("header", { onSetMod: {
		js: { inited: function() {
			this.input = this.findChildBlock(input_default);
			this.search = this.findChildBlock(search_default);
		...
		opened: {
			"true": function() {
				...
				this._domEvents(i_bem_dom_default.doc).on("keydown", function(e) {
					if (e.keyCode === keyboard__codes_default.ESC) {
						this.input.delMod("focused");
						this.delMod("opened");
					}
				});
				if (!mq.matches) this.input.setMod("focused");
			},
			"": function() {
				var _this = this;
				_this._domEvents(i_bem_dom_default.doc).un("keydown");
	...
	i_bem_dom_default.declBlock("yandex-metrica-api", { onSetMod: { js: { inited: function() {
