...
const fs = require('fs');
const prepareLibraryData = require('./prepare-library-data.cjs');
const config = require('./library-config.cjs');
...
module.exports = sourcePage => {
    ...
    versions.forEach(pathToLibData => {
        ...
        const data = prepareLibraryData(pathToLibData, config);

        config.langs.forEach(lang =>
            fs.mkdirSync(path.join(pathToSite, lang), { recursive: true }));
