...
describe('tasks/docs/load-from-github', () => {
    ...
    it('should not process unpublished pages', () => {
        model.setPages([_.extend({}, pageStub, {published: false})]);
        return loadSourceFromGithub(model)().then(() => {
    ...
    it('should not use cache fallback on github API error by default', () => {
        model.setPages([_.extend({}, pageStub)]);
        githubGetContentStub.yields(new Error());
    ...
    it('should use cached source on github API error if "fallbackToCache" option was set', () => {
        const cache = {fileName: '/url/index.ext', sha: 'old-sha'};

        model.setPages([_.extend({}, pageStub)]);
        baseUtil.readFileFromCache.withArgs('/url/index.meta.json', {}).returns(Q(cache));
        baseUtil.readFileFromCache.withArgs('/url/index.ext').returns(Q('cached content'));
        githubGetContentStub.yields(new Error());

        return loadSourceFromGithub(model, {fallbackToCache: true})().then(() => {
            model.getPages()[0].should.have.property('contentFile', '/url/index.ext');
            baseUtil.writeFileToCache.should.not.be.called;
            console.warn.should.be.calledWith('Use cached source for page /url/: /url/index.ext');
        });
    });

    it('should not use github cache fallback if cached source is missing', () => {
        const cache = {fileName: '/url/index.ext', sha: 'old-sha'};

        model.setPages([_.extend({}, pageStub)]);
        baseUtil.readFileFromCache.withArgs('/url/index.meta.json', {}).returns(Q(cache));
        baseUtil.readFileFromCache.withArgs('/url/index.ext').returns(Q.reject('Error'));
        githubGetContentStub.yields(new Error());

        return loadSourceFromGithub(model, {fallbackToCache: true})().then(() => {
            should.not.exist(model.getPages()[0].contentFile);
            baseUtil.writeFileToCache.should.not.be.called;
            console.warn.should.not.be.calledWith('Use cached source for page /url/: /url/index.ext');
        });
    });
