...
describe('tasks/docs/transform-md-html', () => {
    ...
    it('should not transform unpublished markdown pages', () => {
        model.setPages([{url: '/url1/', contentFile: '/some-content.md', published: false}]);
        return transformMdToBemjson(model)().then(() => {
            baseUtil.readFileFromCache.should.not.be.called;
            baseUtil.writeFileToCache.should.not.be.called;
            model.getPages()[0].contentFile.should.equal('/some-content.md');
        });
    });

    it('should not transform unpublished bemjson pages', () => {
        model.setPages([{url: '/url1/', contentFile: '/foo/index.bemjson.js', published: false}]);
        return transformBemjsonToHtml(model)().then(() => {
            baseUtil.readFileFromCache.should.not.be.called;
            baseUtil.writeFileToCache.should.not.be.called;
            model.getPages()[0].contentFile.should.equal('/foo/index.bemjson.js');
        });
    });
