...
describe('tasks/template/full-bem', () => {
    ...
    it('should not compile unpublished pages', () => {
        model.setPages([
            {
                url: '/url1/',
                site: '/url1/',
        ...
        return applyBEM(model, options)().then(() => {
            baseUtil.readFile.should.not.be.called;
            BEMTREE.apply.should.not.be.called;
            BEMHTML.apply.should.not.be.called;
            baseUtil.writeFile.should.not.be.called;
        });
    });
