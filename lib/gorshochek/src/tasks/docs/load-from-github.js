...
module.exports = (model, options) => {
    ...
    function hasGithubSource(page) {
        return baseUtil.isPagePublished(page) && !!(page.source && GITHUB_URL_REGEXP.test(page.source));
    }
    ...
    function saveContentToFile(page, content) {
        ...
        return baseUtil
            .writeFileToCache(filePath, Buffer.from(content.content, 'base64').toString())
    ...
    function getCachedSourceOnError(page, cache, error) {
        if(!options.fallbackToCache || !cache.fileName) {
            return Q.reject(error);
        }

        return baseUtil
            .readFileFromCache(cache.fileName)
            .then(() => {
                console.warn(`Use cached source for page ${page.url}: ${cache.fileName}`);
    ...
    function processPage(model, page) {
        ...
        return readMetaFromCache(page)
            .then(cache => {
                return getContentFromGithubSource(repoInfo, getHeadersByCache(cache))
                    .then(result => ({result, cache}))
                    .catch(error => getCachedSourceOnError(page, cache, error)
                        .then(filePath => ({filePath})));
            })
            .then(({result, cache, filePath}) => {
                if(filePath) {
                    return filePath;
                }
