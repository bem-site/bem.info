...
module.exports = (model, params) => {
    ...
    function findLinkHrefReplacement(linkHref, page, sourceUrlsMap, existedUrls) {
        ...
        */
        ...
        const wasRelative = !util.isGithubUrl(url);
        const fullGithubUrl = wasRelative ?
            resolveFullGithubUrl(linkHref, page.source) :
            linkHref;
        variants.push(fullGithubUrl);
        ...
        if(replacement) {
            ...
            // No bem.info page mirrors this doc — fall back to the upstream
            // GitHub URL so the relative `foo.md` ref opens the actual file
            // instead of 404'ing on bem.info.
            linkHref = fullGithubUrl;
        }
